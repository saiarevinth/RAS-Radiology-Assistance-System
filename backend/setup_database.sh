...
# Database setup script for RAS application
...
DB_NAME="ras_database"
DB_USER="ras_user"
DB_PASSWORD="ras_password"

echo "Creating database: $DB_NAME"
echo "Creating user: $DB_USER"

# Create database and user
psql -U postgres -c "CREATE DATABASE $DB_NAME;" 2>/dev/null || echo "Database already exists or error occurred"
psql -U postgres -c "CREATE USER $DB_USER WITH PASSWORD '$DB_PASSWORD';" 2>/dev/null || echo "User already exists or error occurred"
psql -U postgres -c "GRANT ALL PRIVILEGES ON DATABASE $DB_NAME TO $DB_USER;" 2>/dev/null || echo "Privileges already granted or error occurred"
